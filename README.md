...
The first approach is based on the idea of Higgins et al. in <i>Early Visual Concept Learning with Unsupervised Deep Learning</i>, where the latent vector $z$ is modified, and the changes in the results are observed. The Magnitude of Change ($MC$) is quantified by the relative difference of the Information Gain ($IG$) from the result of the modified latent vector $\hat{z}$ to the original result. 
